...
func (c *Core) quorumRBTConsensus(req *ensweb.Request, did string, qdc didcrypto.DIDCrypto, cr *ConensusRequest) *ensweb.Result {
	...
	*/
	...
	for i := range ti {
		wg.Add(1)
		//go c.checkTokenState(ti[i].Token, did, i, tokenStateCheckResult, &wg, ti[i].BlockID, ti[i].TokenType, address)
		go c.checkTokenState(ti[i].Token, did, i, tokenStateCheckResult, &wg)
	}
	wg.Wait()
	...
	for i := range tokenStateCheckResult {
		...
		if tokenStateCheckResult[i].Status {
